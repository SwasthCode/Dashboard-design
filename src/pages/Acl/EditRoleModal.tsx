...
import { updateRole, Role } from "../../store/slices/roleSlice";
    ...
    useEffect(() => {
        if (role) {
            setFormData({
                name: role.name || "",
                key: role.key || "",
                status: role.status || (role.is_active ? "active" : "inactive"),
                is_active: role.is_active ?? (role.status === "active")
            });
        }
    }, [role]);
        ...
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-lg w-full p-0 overflow-hidden text-outfit">
            <div className="bg-white dark:bg-gray-900">
                ...
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                    <div>
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white">Edit Role</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Modify role details and status.</p>
                    </div>
                </div>
