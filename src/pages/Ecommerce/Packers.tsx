...
import { fetchPackers, deletePacker, Packer } from "../../store/slices/packerSlice";
import Pagination from "../../components/common/Pagination";
import AddPackerModal from './AddPackerModal';
...
export default function Packers() {
    ...
    const { packers, loading } = useSelector((state: RootState) => state.packer);
    ...
    const totalPages = Math.ceil(packers.length / itemsPerPage);
    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const currentPackers = packers.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const handleDeleteClick = (packer: Packer) => {
        setSelectedPacker(packer);
        setIsDeleteModalOpen(true);
    };

    const confirmDelete = async () => {
        if (selectedPacker?._id) {
            setIsDeleting(true);
            try {
                await dispatch(deletePacker(selectedPacker._id)).unwrap();
    ...
    return (
        <div>
            <PageMeta
                title="Packers | Khana Fast Admin"
                description="Manage your packers in the Khana Fast Admin Dashboard"
            />
            <PageBreadcrumb pageTitle="Packers" />
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm">
                                ...
                                <tr className="animate-pulse">
                                    <td colSpan={6} className="px-6 py-10 text-center text-gray-500">
                                        <div className="flex flex-col items-center gap-2">
                                            <DotLoading />
                                            <span>Loading packers...</span>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                currentPackers.map((packer, i) => (
                                    <tr
                                        key={packer._id || i}
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span
                                                className={`px-2 py-1 text-[10px] font-semibold rounded-full uppercase tracking-wider ${packer.status === "active"
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="text-sm text-gray-500 dark:text-gray-400">
                                                {packer.createdAt ? new Date(packer.createdAt).toLocaleDateString() : "-"}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                                            <div className="flex justify-end gap-2">
                                                ...
                                                <button
                                                    onClick={() => handleDeleteClick(packer)}
                ...
                <Pagination
                    ...
                    totalResults={packers.length}
                />
            </div>

            <AddPackerModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} />
            ...
            <DeleteConfirmationModal
                ...
                title="Delete Packer"
                message={`Are you sure you want to delete "${selectedPacker?.name}"? This action cannot be undone.`}
                loading={isDeleting}
            />
        </div>
    );
